...
NEW_VIPRA_MODULE(ORCA, CollisionAvoidance)
{
 public:
  ...
  VIPRA_REGISTER_PARAMS(
      VIPRA_PARAM("radius", _radius), VIPRA_PARAM("time_horizon", _timeHorizon),
      VIPRA_PARAM("maxSpeed", _maxSpeed), VIPRA_PARAM("pref", _timeHorizon));
  ...
  VIPRA_COLLISION_UPDATE
  {
    for ( VIPRA::idx pedIdx = 0; pedIdx < pedset.num_pedestrians(); ++pedIdx ) {
      state.velocities[pedIdx] =
          compute_new_velocity(pedIdx, pedset, deltaT, map);
    }
  }
 ...
 private:
  ...
  [[nodiscard]] auto compute_new_velocity(
      ...
      VIPRA::delta_t timestep, VIPRA::Modules::Map const& map) -> VIPRA::f3d;
