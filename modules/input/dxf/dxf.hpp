...
class DXF : public VIPRA::Modules::Module<DXF>,
 ...
 public:
  ...
  [[nodiscard]] auto get_impl(std::vector<std::string> const& keys) const
      -> std::optional<data_t>
  {
    return std::nullopt;
  }
...
inline void VIPRA::Input::DXF::load_impl()
{
  ...
  if ( ! std::filesystem::is_regular_file(_filepath) )
    throw std::runtime_error("File is not a regular file at: " +
                             _filepath.string());
