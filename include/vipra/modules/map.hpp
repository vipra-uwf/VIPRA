...
class Map {
 public:
  ...
  {
    return std::any_of(_obstacles.begin(), _obstacles.end(), [&](auto const& obstacle) {
      return obstacle.is_point_inside(point);
    });
  }
  [[nodiscard]] virtual auto collision(VIPRA::Geometry::Circle const& radius) const
      -> bool
  {
  ...
  {
    VIPRA::Log::debug("Adding Obstacle at {}, {}, {}", obstacle.center().x,
                      obstacle.center().y, obstacle.center().z);
  ...
  {
    VIPRA::Log::debug("Adding Objective at {}, {}, {}", objective.center().x,
                      objective.center().y, objective.center().z);
  ...
  {
    VIPRA::Log::debug("Adding Spawn at {}, {}, {}", spawn.center().x, spawn.center().y,
                      spawn.center().z);
    _spawns.push_back(spawn);
  }
  void add_area(std::string const& name, VIPRA::Geometry::Polygon const& polygon)
  {
    VIPRA::Log::debug("Adding Area at {}, {}, {}", polygon.center().x, polygon.center().y,
                      polygon.center().z);
    _areas[name] = polygon;
      ...
      -> std::vector<VIPRA::Geometry::Polygon> const&
  {
    ...
    if ( iter == _objectives.end() ) {
      VIPRA_BASE_MODULE_ERROR("Map", "Map does NOT have objectives of type: {}", type);
    }
  ...
  [[nodiscard]] auto get_spawns() const -> std::vector<VIPRA::Geometry::Polygon> const&
  {
    return _spawns;
  }
  [[nodiscard]] auto get_areas() const -> std::map<std::string, VIPRA::Geometry::Polygon>
  {
    return _areas;
  }
  ...
  void initialize(VIPRA::Modules::MapInput& input)
  {
    ...
    if ( ! obstacles )
      VIPRA_BASE_MODULE_ERROR("Map", "Input Module failed to load map obstacles");

    auto objectives = input.get_objectives();
    if ( ! objectives )
      VIPRA_BASE_MODULE_ERROR("Map", "Input Module failed to load map objectives");
    ...
    VIPRA::Log::debug("Map: {} Obstacles, {} Spawns, {} Areas", _obstacles.size(),
                      _spawns.size(), _areas.size());
 ...
 private:
      ...
      std::map<std::string, VIPRA::Geometry::Polygon> const&              areas)
  {
    ...
    VIPRA::Log::debug("Map Dimensions: {}, {}, {}", _dimensions.x, _dimensions.y,
                      _dimensions.z);
  }
