...
class Obstacles : public Util::CRTP<Obstacles<module_t>> {
 ...
 public:
  void initialize(auto&& input)
  {
    ...
    auto const objTypes =
        input.template get<std::vector<std::string>>("obj_types");
    if ( ! objTypes )
      throw std::runtime_error("Could not find object types in input file");
    ...
    std::for_each(objTypes->begin(), objTypes->end(), [&](auto const& objType) {
      const auto positions =
          input.template get<std::vector<VIPRA::f3d>>(objType);
      if ( ! positions ) {
    ...
    if ( ! obsCoords )
      throw std::runtime_error("Could not get obstacle polygons from input");
  ...
  [[nodiscard]] auto ray_hit(VIPRA::f3d start,
  ...
  [[nodiscard]] auto get_objects(std::string const& type) const
      -> const VIPRA::f3dVec&
  {
