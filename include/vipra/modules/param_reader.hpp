...
class ParamReader : public Util::CRTP<ParamReader<module_t>> {
 ...
 public:
  ...
  auto get_param(std::string const& type, std::string const& moduleName, std::string const& paramName,
    ...
    auto singleValue = this->self().template get<data_t>(type, moduleName, paramName);
    if ( singleValue ) {
      return singleValue;
    }

    auto arrayValue = this->self().template get<std::vector<data_t>>(type, moduleName, paramName);
    if ( arrayValue ) {
      return get_discrete_value<data_t>(arrayValue.value(), engine);
    }

    auto mapValue = this->self().template get<std::map<std::string, data_t>>(type, moduleName, paramName);
    if ( arrayValue ) {
