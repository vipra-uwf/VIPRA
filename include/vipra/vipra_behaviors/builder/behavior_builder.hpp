...
class BehaviorBuilder : public BehaviorBaseVisitor {
 ...
 private:
  ...
  void add_atom_to_action(Action&                             action,
                          BehaviorParser::Action_atomContext* atom);
  void add_target_to_action(Action& action, BehaviorParser::TargetContext* ctx);

  [[nodiscard]] auto add_event(BehaviorParser::Event_nameContext* ctx)
      -> VIPRA::idx;

  [[nodiscard]] auto build_condition(BehaviorParser::ConditionContext* cond)
      -> Condition;
  void condition_tree_condition(BehaviorParser::ConditionContext* condition,
                                Condition&                        tree);
  void condition_tree_unary(BehaviorParser::UnaryContext* unary,
                            Condition&                    tree);
  void condition_tree_primary(BehaviorParser::PrimaryContext* primary,
                              Condition&                      tree);
  ...
  [[nodiscard]] auto get_composite_type(
      ...
      -> Behaviors::Ptype;
  [[nodiscard]] static auto get_attribute(std::string attr)
      -> Behaviors::Attribute;

  [[nodiscard]] auto get_check_location(std::string const& name) const
      -> VIPRA::idx;
  [[nodiscard]] auto get_check_state(std::string const& name) const
      -> Behaviors::stateUID;
  [[nodiscard]] auto get_check_event(std::string const& name) const
      -> VIPRA::idx;
  [[nodiscard]] auto get_check_type(std::string const& name) const
      -> Behaviors::typeUID;

  [[nodiscard]] auto make_attribute_value(
      BehaviorParser::Attr_valueContext* ctx) -> Behaviors::CAttributeValue;
  [[nodiscard]] static auto make_attribute_str(
  ...
  [[nodiscard]] auto make_target_modifier(
      std::vector<BehaviorParser::ModifierContext*>& modifiers)
      -> std::optional<TargetModifier>;
  [[nodiscard]] static auto make_direction(
      BehaviorParser::DirectionContext* ctx) -> Direction;
  [[nodiscard]] auto make_dimensions(BehaviorParser::Loc_dimensionsContext* ctx)
      const -> std::tuple<VIPRA::f3d, VIPRA::f3d, VIPRA::f_pnt>;
  ...
  auto visitEvent(BehaviorParser::EventContext* /*ctx*/)
      -> antlrcpp::Any override;
  auto visitPed_Selector(BehaviorParser::Ped_SelectorContext* /*ctx*/)
      -> antlrcpp::Any override;
  auto visitAction(BehaviorParser::ActionContext* /*ctx*/)
      -> antlrcpp::Any override;
  auto visitDecl_Ped_State(BehaviorParser::Decl_Ped_StateContext* /*ctx*/)
      -> antlrcpp::Any override;
  auto visitDecl_Ped(BehaviorParser::Decl_PedContext* /*ctx*/)
      -> antlrcpp::Any override;
  auto visitLocation(BehaviorParser::LocationContext* /*ctx*/)
      -> antlrcpp::Any override;
  ...
  auto build_everyone_selector(slType /*type*/,
                               bool /*required*/) -> SubSelector;
  ...
  [[nodiscard]] static auto find_event_component(
      BehaviorParser::EventContext* ctx) -> std::optional<comp_t>
  {
   ...
   */
  template <typename comp_t>
  [[nodiscard]] static auto find_action_component(
      BehaviorParser::ActionContext* ctx) -> std::optional<comp_t>
  {
   ...
   */
  template <typename comp_t>
  [[nodiscard]] static auto find_location_component(
      BehaviorParser::LocationContext* ctx) -> std::optional<comp_t>
  {
...
void BehaviorBuilder::end_behavior_check()
{
  if ( _currentBehavior.selector_count() == 0 ) {
    VIPRA::Log::error(
        "Behavior Error: No Pedestrian Selector Defined For Behavior: \"{}\"",
        _currentBehavior.get_name());
    BuilderException::error();
  }
  if ( _currentBehavior.action_count() == 0 ) {
    VIPRA::Log::warn("Behavior Error: No Actions Defined For Behavior: \"{}\"",
                     _currentBehavior.get_name());
  }
}
    ...
    Condition& condTree, BehaviorParser::Sub_conditionContext* subcond)
{
  ...
  if ( subcond->condition_Event_Occurred() ) {
    condTree.add_subcondition(
        build_event_occurred_subcond(subcond->condition_Event_Occurred()));
    return;
  }

  if ( subcond->condition_Event_Occurring() ) {
    condTree.add_subcondition(
        build_event_occurring_subcond(subcond->condition_Event_Occurring()));
    return;
  }

  if ( subcond->condition_Event_Starting() ) {
    condTree.add_subcondition(
        build_event_starting_subcond(subcond->condition_Event_Starting()));
    return;
  }

  if ( subcond->condition_Event_Ending() ) {
    condTree.add_subcondition(
        build_event_ending_subcond(subcond->condition_Event_Ending()));
    return;
  }
  ...
  if ( subcond->condition_Inside_Location() ) {
    condTree.add_subcondition(
        build_in_location_subcond(subcond->condition_Inside_Location()));
    return;
  }
  ...
  VIPRA::Log::error("Behavior Error: No Valid SubCondition For: \"{}\"",
                    subcond->getText());
  BuilderException::error();
}

void BehaviorBuilder::condition_tree_condition(
...
void BehaviorBuilder::condition_tree_unary(BehaviorParser::UnaryContext* unary,
                                           Condition&                    tree)
{
  // if (unary->unary()) {
...
auto BehaviorBuilder::get_state(std::string const& state) const
    -> std::optional<Behaviors::stateUID>
{
...
auto BehaviorBuilder::get_location(std::string const& locName) const
    -> std::optional<VIPRA::idx>
{
...
auto BehaviorBuilder::get_event(std::string const& evName) const
    -> std::optional<VIPRA::idx>
{
...
auto BehaviorBuilder::get_attribute(std::string attr) -> Attribute
{
  static std::map<std::string, Attribute> attrMap{
      {"position", Attribute::POSITION}, {"end goal", Attribute::END_GOAL},
      {"goal", Attribute::CURR_GOAL},    {"state", Attribute::STATE},
      {"velocity", Attribute::VELOCITY}, {"location", Attribute::LOCATION},
...
auto BehaviorBuilder::get_check_state(std::string const& stateName) const
    -> Behaviors::stateUID
{
  VIPRA::Log::debug("GET STATE: {}", stateName);
  auto state = get_state(stateName);
  if ( ! state )
    error("Behavior Error: Attempt To Use Undeclared State: \"{}\"", stateName);
  return state.value();
}
    ...
    -> VIPRA::idx
{
  ...
  if ( ! event )
    error("Behavior Error: Attempt To Use Undeclared Event: \"{}\"", eventName);
  return event.value();
}
...
auto BehaviorBuilder::get_check_type(std::string const& typeName) const
    -> Behaviors::typeUID
{
    ...
    BehaviorParser::Attr_valueContext* ctx) -> Behaviors::CAttributeValue
{
  if ( ctx->value_coord() ) {
    return AttributeHandling::store_value<VIPRA::f3d>(
        Type::COORD, get_coord(ctx->value_coord(), _currSeed));
  }

  if ( ctx->STATE_VAL() ) {
    return AttributeHandling::store_value<Behaviors::stateUID>(
        Type::STATE, get_check_state(ctx->STATE_VAL()->toString()));
  }
...
auto BehaviorBuilder::make_attribute_str(BehaviorParser::AttributeContext* ctx)
    -> std::string
{
  if ( ctx->POSITION() ) {
    return "position";
    ...
    -> VIPRA::idx
{
  ...
  VIPRA::Log::debug(R"(Behavior "{}": Adding Lasting Event: "{}")",
                    _currentBehavior.get_name(), eventName);
    ...
    -> antlrcpp::Any
{
  ...
  if ( ! name )
    error(R"(Behavior "{}": Missing Event Name)", _currentBehavior.get_name());
  std::string eventName = "!" + name.value();

  if ( get_event(eventName) )
    error(R"(Behavior "{}": Attempt To Redefine Event: "{}")",
          _currentBehavior.get_name(), eventName);
  ...
  VIPRA::Log::debug(R"(Behavior "{}": Event: "{}" Adding Start Condition)",
                    _currentBehavior.get_name(), eventName);
  event.set_start_condition(build_condition(startCond.value()->condition()));

  if ( endCond ) {
    VIPRA::Log::debug(R"(Behavior "{}": Event: "{}" Adding End Condition)",
                      _currentBehavior.get_name(), eventName);
    ...
    -> antlrcpp::Any
{
  ...
  if ( duration )
    action.add_duration(
        get_numeric(duration.value()->duration()->value_numeric(), _currSeed));
    ...
    BehaviorParser::Decl_Ped_StateContext* ctx) -> antlrcpp::Any
{
  ...
  for ( auto* state : stateNames ) {
    ...
    if ( _states.contains("#" + name) ) {
      VIPRA::Log::error("Behavior \"{}\": Duplicate State {}",
                        _currentBehavior.get_name(), name);
      BuilderException::error();
    }

    VIPRA::Log::debug("Behavior \"{}\": Adding Pedestrian State {}, id: {}",
                      _currentBehavior.get_name(), name,
                      static_cast<uint64_t>(_currState));
    ...
    -> antlrcpp::Any
{
  ...
  for ( auto* type : typeNames ) {
    ...
    VIPRA::Log::debug("Behavior \"{}\": Adding Pedestrian Type {}, id: {}",
                      _currentBehavior.get_name(), name, _currType);
...
void BehaviorBuilder::add_scale_atom(Action&                            action,
                                     BehaviorParser::Scale_atomContext* ctx)
{
  auto attrStr = make_attribute_str(ctx->attribute());
  auto attr = get_attribute(attrStr);
...
auto BehaviorBuilder::build_exit_subcond(
...
auto BehaviorBuilder::build_attribute_subcond(
    BehaviorParser::Condition_AttributeContext* ctx) -> SubConditionAttribute
{
  ...
  VIPRA::Log::debug(R"(Behavior "{}": Adding SubCondition: Attribute "{}")",
    ...
    std::optional<TargetModifier>& modifier)
{
  ...
  VIPRA::Log::debug(R"(Behavior: "{}": Adding Target: Nearest "{}")",
                    _currentBehavior.get_name(), fmt::join(typeStrs, " or "));
  action.add_target(
      TargetSelector(TargetNearest{comPtype, allPeds, std::move(modifier)}));
}
...
auto BehaviorBuilder::build_exactly_n_selector(slType type, slSelector selector,
                                               std::optional<slGroup> group,
                                               bool required) -> SubSelector
{
  auto  types = type->id_list()->ID();
...
auto BehaviorBuilder::build_percent_selector(slType type, slSelector selector,
                                             std::optional<slGroup> group,
                                             bool required) -> SubSelector
{
  auto  types = type->id_list()->ID();
...
auto BehaviorBuilder::build_location_selector(slType type, slSelector selector,
                                              std::optional<slGroup> group,
                                              bool required) -> SubSelector
{
  auto  types = type->id_list()->ID();
